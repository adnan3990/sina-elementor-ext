			...
			<div class="sina-portfolio-btns">
				...
				<?php
					...
					foreach ($data['portfolio'] as $key => $item) :
						$category = strtolower( str_replace( ' ', '', $item['category'] ) );
					?>
					<?php
						if ( !in_array($category, $category_in) && $category ) :
							$category_in[] = $category;
						?>
						<button class="sina-portfolio-btn sina-button" data-filter=".<?php echo esc_attr( $category ); ?>"><?php echo esc_html( $item['category'] ); ?></button>
					<?php endif ?>
				<?php endforeach; ?>
			</div>
